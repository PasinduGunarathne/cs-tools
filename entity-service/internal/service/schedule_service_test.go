...
func TestSearchAssignmentsRejectsBadWindows(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := NewScheduleService(repo).SearchAssignments(context.Background(), tc.req)
...
func TestSearchAssignmentsPassesTheRequestThrough(t *testing.T) {
	...
	resp, err := NewScheduleService(repo).SearchAssignments(context.Background(), req)
	if err != nil {
...
func TestSearchAssignmentsAcceptsAWindowAtTheLimit(t *testing.T) {
	...
	svc := NewScheduleService(&fakeScheduleRepo{})
...
func TestSearchAbsencesValidatesTheSameWindow(t *testing.T) {
	...
	repo := &fakeScheduleRepo{}
	_, err := NewScheduleService(repo).SearchAbsences(context.Background(),
...
func TestOnDutyDefaultsToNow(t *testing.T) {
	...
	repo := &fakeScheduleRepo{}
	before := time.Now()
	if _, err := NewScheduleService(repo).OnDuty(context.Background(), nil); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestOnDutyUsesTheInstantAskedFor(t *testing.T) {
	...
	if _, err := NewScheduleService(repo).OnDuty(context.Background(), &at); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestCatalogueIsServedStraightThrough(t *testing.T) {
	...
	cat, err := NewScheduleService(repo).Catalogue(context.Background())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
