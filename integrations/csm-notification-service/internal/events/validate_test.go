...
func TestValidate_RequiresFields(t *testing.T) {
	cases := map[string]struct {
		...
		"sla.clock.register missing caseId":         {"CASE-1", TypeSLAClockRegister, `{"caseTitle":"Gateway returning 500s","durations":{"response":"2h"}}`},
		// slaengine stores caseTitle on the registered clock and renders it in
		// the breach Chat card, and entity-service always publishes it
		// (sn_case_service.go's CaseTitle: cv.Subject). A payload without one
		// would produce a breach alert with a blank subject line.
		"sla.clock.register missing caseTitle":        {"CASE-1", TypeSLAClockRegister, `{"caseId":"CASE-1","durations":{"response":"2h"}}`},
		"sla.clock.register missing durations":        {"CASE-1", TypeSLAClockRegister, `{"caseTitle":"Gateway returning 500s","caseId":"CASE-1","durations":{}}`},
		"sla.clock.register empty clock type":         {"CASE-1", TypeSLAClockRegister, `{"caseTitle":"Gateway returning 500s","caseId":"CASE-1","durations":{"":"2h"}}`},
		"sla.clock.register empty duration":           {"CASE-1", TypeSLAClockRegister, `{"caseTitle":"Gateway returning 500s","caseId":"CASE-1","durations":{"response":""}}`},
		"sla.clock.register caseId/entityId mismatch": {"CASE-1", TypeSLAClockRegister, `{"caseTitle":"Gateway returning 500s","caseId":"CASE-2","durations":{"response":"2h"}}`},
		"sla.clock.register unparsable duration":      {"CASE-1", TypeSLAClockRegister, `{"caseTitle":"Gateway returning 500s","caseId":"CASE-1","durations":{"response":"not-a-duration"}}`},
		"sla.clock.register zero duration":            {"CASE-1", TypeSLAClockRegister, `{"caseTitle":"Gateway returning 500s","caseId":"CASE-1","durations":{"response":"0s"}}`},
		"sla.clock.register negative duration":        {"CASE-1", TypeSLAClockRegister, `{"caseTitle":"Gateway returning 500s","caseId":"CASE-1","durations":{"response":"-2h"}}`},
