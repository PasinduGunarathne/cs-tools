...
func TestEngine_Handle_RegistersClockAndSeedsWakeEntries(t *testing.T) {
	...
	record := eventbus.Record{Value: []byte(`{"type":"sla.clock.register","entityId":"CASE-1","payload":{"caseTitle":"Gateway returning 500s","caseId":"CASE-1","durations":{"response":"2h"}}}`)}
	if err := e.Handle(context.Background(), record); err != nil {
		t.Fatalf("Handle() error = %v, want nil", err)
	}
...
func TestEngine_Handle_RejectsWholeRecordOnOneInvalidDuration(t *testing.T) {
	...
	record := eventbus.Record{Value: []byte(`{"type":"sla.clock.register","entityId":"CASE-1","payload":{"caseTitle":"Gateway returning 500s","caseId":"CASE-1","durations":{"response":"not-a-duration","resolution":"4h"}}}`)}
	if err := e.Handle(context.Background(), record); err == nil {
